            ...
            <table class="table table-hover" id="customerTable">
              ...
              <c:if test="${not empty userList}">
                <c:forEach var="user" items="${userList}" varStatus="loop">
                  <tr data-id="${user.id}"
                      data-name="${not empty user.name ? user.name : ''}"
              ...
              <c:if test="${empty userList or userList.size() == 0}">
                <tr>
                  <td colspan="7" class="text-center">Không có khách hàng nào. (Debug: userList size = ${userList != null ? userList.size() : 0})</td>
                </tr>
              </c:if>
              </tbody>
            </table>
          ...
          <div class="row">
            ...
            <div class="col-md-9">
              <div class="mb-3">
                ...
                <input type="text" class="form-control form-control-sm" id="customerName" name="customerName" required>
              </div>
              <div class="mb-3">
                <label for="customerPhone" class="form-label small">Điện thoại</label>
                <input type="text" class="form-control form-control-sm" id="customerPhone" name="customerPhone" required>
              </div>
              <div class="mb-3">
                <label for="customerBirthdate" class="form-label small d-block">Ngày sinh</label>
                <div class="input-group input-group-sm">
                  <input type="date" class="form-control form-control-sm" id="customerBirthdate" name="customerBirthdate"
        ...
        detailCell.innerHTML = `
          <div class="customer-detail-container p-3">
            <div class="row mb-3">
              ...
              <div class="col">
                <h5>${name || 'Chưa có'}</h5>
                <p>Mã: ${id ? "KH" + id.substring(0, 4).toUpperCase() : 'Chưa có'}</p>
              </div>
            </div>
            <div class="row mb-3">
              <div class="col-md-4"><p class="small mb-1 text-muted">Điện thoại</p><p>${phone || 'Chưa có'}</p></div>
              <div class="col-md-4"><p class="small mb-1 text-muted">Sinh nhật</p><p>${birthday || 'Chưa có'}</p></div>
              <div class="col-md-4"><p class="small mb-1 text-muted">Giới tính</p><p>${gender || 'Chưa có'}</p></div>
            </div>
            <div class="row mb-3">
              <div class="col-md-4"><p class="small mb-1 text-muted">Email</p><p>${email || 'Chưa có'}</p></div>
              <div class="col-md-8"><p class="small mb-1 text-muted">Địa chỉ</p><p>${address || 'Chưa có'}</p></div>
            </div>
            <hr>
    ...
    function deleteCustomer(id) {
      if (confirm('Bạn có chắc chắn muốn xóa khách hàng này?')) {
        fetch('/Aishiba/customer', {
          ...
          body: `action=delete&customerId=${id}`
        })
